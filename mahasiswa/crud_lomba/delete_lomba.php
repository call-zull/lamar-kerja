...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    try {
        ...
        if ($result) {
            ...
            // Delete lomba record from database
            $stmt = $pdo->prepare("DELETE FROM lomba WHERE id = :id AND mahasiswa_id = :mahasiswa_id");
            $stmt->bindParam(':id', $idLomba, PDO::PARAM_INT);
            $stmt->bindParam(':mahasiswa_id', $mahasiswa_id, PDO::PARAM_INT);

            if ($stmt->execute()) {
                $_SESSION['success_message'] = "Data lomba berhasil dihapus.";
                header("Location: tampil_lomba.php");
                exit();
            } else {
                $_SESSION['error_message'] = "Gagal menghapus data lomba.";
                header("Location: tampil_lomba.php");
                exit();
            }
        } else {
        ...
        $_SESSION['error_message'] = "Terjadi kesalahan saat menghapus data lomba: " . $e->getMessage();
        header("Location: tampil_lomba.php");
        exit();
    }
} else {
